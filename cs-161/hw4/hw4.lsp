...
;
; i think we get to assume inputs are valid
(defun at-least-one-color (n c k)
  (cond
    ; base case
    ((> c k) nil)
    ; otherwise add current index to clause (splits by or)
    (t (cons (node2var n c k) (at-least-one-color n (+ c 1) k)))
  )
)

; the following is the helper for exactly-this-color, it's not actually
; "at least one color" but i dont wanna work through the logic of it rn
(defun at-least-one-color-list (n c k)
  (cond
    ; base case
    ((> c k) nil)
    ; otherwise add current index to clause (splits by or)
    (t (cons
      (list (node2var n c k))
      (at-least-one-color-list n (+ c 1) k)
      )
    )
  )
)
...
(defun at-most-one-color (n c k)
  (at-most-one-color-improved n c c k) ; include starting number
)

(defun at-most-one-color-improved (n c m k)
  (cond
    ((> m k) nil)
    (t
      (append
        (list (exactly-this-color n c m k))
        (at-most-one-color-improved n c (+ m 1) k)
      )
    )
  )
)

(defun exactly-this-color (n c m k)
  (append
    (list (list (node2var n m m)))
    (at-least-one-color-list n (- 1 m) (- c))
    (at-least-one-color-list n (- k) (- (+ 1 m)))
  )
)
...
(defun generate-node-clauses (n k)
  (list (at-least-one-color n 1 k) (at-most-one-color n 1 k))
)
...
(defun generate-edge-clauses (e k)
  (generate-edge-clauses-improved e 1 k) ; 1 starts the iterator
)
(defun generate-edge-clauses-improved (e m k)
  ; de morgan's laws: not (A and B) = (not A) or (not B)
  ; if a overlaps with b then node2var a = node2var b
  ; the only allowable combos are ((not node2var a) or (not node2var b)) AND ...
  (cond
    ((> m k) nil)
    (t
      (append
        (list (- (node2var (car e) m k)) (- (node2var (cadr e) m k)))
        (generate-edge-clauses-improved e (+ m 1) k)
      )
    )
  )
)
